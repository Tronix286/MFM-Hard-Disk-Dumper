...
//   uint16_t deltas[(131072L-4700L)];
   uint16_t deltas[131072L];
   // Size of shared memory/deltas in bytes
   int ddr_mem_size;
   char *cmdline;
//   int max_deltas=(131072L-4700L);
   int max_deltas=131072;
...
*/
   ...
   if (drive_params.analyze) {
	...
	//FILE* fp;
	//fp = fopen("pico.bin", "rb");
	//fread(deltas, (131072-4700), 2, fp); // Read in the entire file
	//fclose(fp); // Close the file

      int head_cmdline = drive_params.num_head;
      int cyl_cmdline = drive_params.num_cyl;
	
	printf("cyl = %u; head = %u\r\n",drive_params.analyze_cyl,drive_params.analyze_head);
