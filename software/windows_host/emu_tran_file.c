...
int pread(FILE* f, void *buf, int count, int offset)
{
     int c;
     int64_t os, curroff = ftell(f);
     ...
     os = fseek(f, curroff, 0);
     if (os < 0) return os;
    
     return c;
}
int pwrite(FILE* f, void *buf, int count, int offset)
{	
    int c;
    int64_t os, curroff = ftell(f);
    if (curroff < 0) return curroff;
...
void emu_file_rewrite_track(FILE* fd, EMU_FILE_INFO *emu_file_info,
...
// buf_size: Buffer size in bytes
void emu_file_read_cyl(FILE* fd, EMU_FILE_INFO *emu_file_info, int cyl,
...
// buf: Buffer to write
// buf_size: Buffer size in bytes
void emu_file_write_cyl(FILE* fd, EMU_FILE_INFO *emu_file_info, int cyl,
      void *buf, int buf_size) {
   off_t offset;
   int rc;
   int cyl_size = emu_file_info->num_head *
